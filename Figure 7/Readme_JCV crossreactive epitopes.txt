...
# Project Description
To identify cross-reactivity of JCV epitopes across different strains of BKV
...
Usage: Execute this script after placing the respective Excel files (JCV_scored_epitopes.xlsx and BKV_strains_protein_sequences.xlsx) in the same directory. It generates result files: results_JCV_crossreactive_epitopes.xlsx and pivot_table_JCV_crossreactive_epitopes.xlsx.

Instructions:
Running the Script:

Ensure all required packages are installed (Pandas, Seaborn).
Place the input Excel files (JCV_scored_epitopes.xlsx and BKV_strains_protein_sequences.xlsx) in the same directory as the script.
Run the script to perform epitope matching and pivot table generation for cross-reactive epitopes.
Input Files:

JCV_scored_epitopes.xlsx: Contains epitopes for JCV.
BKV_strains_protein_sequences.xlsx: Contains protein sequences for JCV strains.
Output Files:

Upon execution, the script generates two output files:
results_JCV_crossreactive_epitopes.xlsx: Epitopes matched with gene names and strains.
pivot_table_JCV_crossreactive_epitopes.xlsx: Pivot table summarizing epitope occurrences across strains.
Adjustments:
